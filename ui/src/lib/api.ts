...
// API_BASE is used for fetches.
// We use a relative path which works both in the browser and in SvelteKit load functions.
...
export function getPublicApiBase() {
	...
	// Fallback for server-side if window is not available.
	// In most cases, the relative path is sufficient for internal fetches.
	return API_BASE;
}
...
export async function removeTarget(
	...
	const res = await fetchFn(
		`${API_BASE}/groups/${groupId}/targets/${encodeURIComponent(address)}`,
		{
			method: 'DELETE'
		}
	);
